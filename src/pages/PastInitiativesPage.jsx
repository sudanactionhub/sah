...
const InitiativeCard = ({ initiative }) => {
  const imageContent = initiative.image_url ? (
    <img
    ...
    <div className="w-full h-full bg-white flex items-center justify-center text-gray-400 text-sm">
      No Image
    </div>
  );

  const displayDate = initiative.date ? format(parseISO(initiative.date), 'MMMM d, yyyy') : '';
  ...
  if (initiative.link) {
    if (initiative.link.startsWith('/')) {
      return (
        <Link to={initiative.link} className="block h-full">
          {cardContent}
        </Link>
      );
    }
  ...
  const filteredInitiatives = useMemo(() => {
    return initiatives.filter((initiative) => {
      ...
      const matchesDate = dateFilter
        ? isSameDay(parseISO(initiative.date), parseISO(dateFilter))
        : true;
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Explore our impactful campaigns, events, and programming dedicated to supporting Sudan. See the difference we've made together."
        />
        <meta
          name="keywords"
          content="Sudan initiatives, Sudan events, advocacy history, humanitarian initiatives, Sudan Action Hub, programming"
        />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            ...
            <p className="text-xl text-green-100 max-w-3xl mx-auto">
              Efforts to drive change and support Sudan across the United States.
            </p>
          </motion.div>
        </div>
      </div>
